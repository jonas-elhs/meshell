...
def parse_args() -> (argparse.ArgumentParser, argparse.Namespace):
  ...
  # subcommands = parser.add_subparsers(title="subcommands", metavar="COMMAND")

  # Bar
  bar_parser = subcommands.add_parser("bar", help="control the bar")
  bar_parser.set_defaults(execute=bar.toggle)

  bar_subparsers = bar_parser.add_subparsers(title="actions", metavar="ACTION")
  ...
  power_parser.set_defaults(execute=powermenu.toggle)
